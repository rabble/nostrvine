// ABOUTME: Tests that ContentBlocklistRepository uses publishEventWithRetry
...
class _MockAuthService extends Mock implements AuthService, BlockListSigner {}
...
void main() {
  ...
  Future<ContentBlocklistRepository> buildService() async {
  ...
  group('ContentBlocklistRepository reliability', () {
    test(
    ...
    test('unblock: rollback keeps user blocked when relay rejects', () async {
      when(
        () => nostr.publishEventWithRetry(
          any(),
          policy: any(named: 'policy'),
    ...
    test(
      ...
      () async {
        ...
        final result = await service.blockUser(ourPubkey, ourPubkey: ourPubkey);
