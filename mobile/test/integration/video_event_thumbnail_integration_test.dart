...
void main() {
  group('VideoEvent Thumbnail API Integration', () {
    ...
    const realVideoId = '87444ba2b07f28f29a8df3e9b358712e434a9d94bc67b08db5d4de61e6205344';
    ...
    group('effectiveThumbnailUrl behavior', () {
      test('returns existing thumbnail when available', () {
        ...
        expect(thumbnailUrl, equals('https://existing-thumbnail.com/thumb.jpg'));
      });
    ...
    group('getApiThumbnailUrl async method', () {
      ...
      test('attempts API generation when no thumbnail available', () async {
        final thumbnailUrl = await videoEventWithoutThumbnail.getApiThumbnailUrl(
          timeSeconds: 2.0,
        ...
        if (thumbnailUrl != null) {
          ...
          print('Successfully generated API thumbnail: $thumbnailUrl');
        } else {
          print('API thumbnail generation returned null (expected in test environment)');
        }
        
        expect(thumbnailUrl, anyOf(isNull, isA<String>()));
      }, timeout: const Timeout(Duration(seconds: 30)));

      test('respects size parameter in API call', () async {
        final thumbnailUrl = await videoEventWithoutThumbnail.getApiThumbnailUrl(
          timeSeconds: 1.5,
    ...
    group('Thumbnail workflow comparison', () {
      test('sync vs async methods provide consistent URLs when no generation needed', () async {
      ...
      test('sync method provides immediate URL while async attempts generation', () async {
    ...
    group('Error handling and edge cases', () {
      ...
      test('async method handles network errors gracefully', () async {
    ...
    group('Backward compatibility', () {
      test('effectiveThumbnailUrl still works as before', () {
        ...
        expect(testVideoEvent.effectiveThumbnailUrl, equals('https://existing-thumbnail.com/thumb.jpg'));
    ...
    group('Performance considerations', () {
      test('sync method is fast', () {
        ...
        print('100 sync calls took ${duration}ms (${duration / 100}ms average)');
        expect(duration, lessThan(100)); // Should be very fast
      });

      test('async method timeout handling', () async {
        ...
        print('Async call took ${duration.inMilliseconds}ms');
