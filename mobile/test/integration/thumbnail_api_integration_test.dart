...
void main() {
  group('ThumbnailApiService Integration Tests', () {
    // Test configuration
    const testVideoId = 'test-video-12345'; // Use a real video ID from your test data
    const realVideoId = '87444ba2b07f28f29a8df3e9b358712e434a9d94bc67b08db5d4de61e6205344'; // Rabble's video
    ...
    group('Real API Server Tests', () {
      ...
      test('thumbnailExists works with real video', () async {
        ...
        print('Thumbnail exists for $realVideoId: $exists');
      }, timeout: const Timeout(Duration(seconds: 30)));

      test('thumbnailExists returns false for non-existent video', () async {
        final exists = await ThumbnailApiService.thumbnailExists(nonExistentVideoId);
        expect(exists, isFalse);
      }, timeout: const Timeout(Duration(seconds: 30)));

      test('getThumbnailWithFallback works with real video URL', () async {
        // Test thumbnail generation for Rabble's known video
        final url = await ThumbnailApiService.getThumbnailWithFallback(realVideoId, timeSeconds: 1.0);
        
        if (url != null) {
          expect(url, startsWith('https://api.openvine.co/thumbnail/'));
          expect(url, contains(realVideoId));
          expect(url, contains('t=1.0'));
          print('Successfully generated thumbnail: $url');
        } else {
          print('Thumbnail generation failed for $realVideoId - this may be expected if already exists');
        }
      ...
      test('getThumbnailWithFallback handles non-existent video gracefully', () async {
        final url = await ThumbnailApiService.getThumbnailWithFallback(nonExistentVideoId);
        expect(url, isNull);
      }, timeout: const Timeout(Duration(seconds: 30)));

      test('getThumbnailWithFallback works end-to-end', () async {
        ...
        if (url != null) {
          expect(url, startsWith('https://api.openvine.co/thumbnail/'));
          expect(url, contains(realVideoId));
          print('Got thumbnail with fallback: $url');
        } else {
          print('Fallback thumbnail generation failed for $realVideoId');
        }
      ...
      test('uploadCustomThumbnail handles file upload', () async {
        ...
        print('Custom thumbnail upload result: $success');
        // Should return a boolean, but not throw
        expect(success, isA<bool>());
      }, timeout: const Timeout(Duration(seconds: 60)));

      test('batchGenerateThumbnails handles multiple videos', () async {
        ...
        print('Batch generation results: ${results.length}/${videoIds.length} successful');
    ...
    group('Error Handling Tests', () {
      test('handles malformed video IDs gracefully', () async {
        const malformedId = 'invalid@video#id!';
        
        final exists = await ThumbnailApiService.thumbnailExists(malformedId);
        expect(exists, isFalse);

        final url = await ThumbnailApiService.getThumbnailWithFallback(malformedId);
        expect(url, isNull);
      }, timeout: const Timeout(Duration(seconds: 30)));

      test('handles extremely long video IDs', () async {
        final longId = 'a' * 1000;
        
        final exists = await ThumbnailApiService.thumbnailExists(longId);
        expect(exists, isFalse);

        final url = await ThumbnailApiService.getThumbnailWithFallback(longId);
        expect(url, isNull);
      }, timeout: const Timeout(Duration(seconds: 30)));
      ...
      test('handles special characters in video ID', () async {
        ...
        final url = await ThumbnailApiService.getThumbnailWithFallback(specialId);
        expect(url, anyOf(isNull, isA<String>()));
      }, timeout: const Timeout(Duration(seconds: 30)));
    });

    group('Performance Tests', () {
      test('thumbnail generation completes within reasonable time', () async {
        ...
        print('Thumbnail generation took ${duration}ms');
        expect(duration, lessThan(30000)); // Should complete within 30 seconds
      }, timeout: const Timeout(Duration(seconds: 45)));

      test('batch processing is reasonably efficient', () async {
        ...
        print('Batch processing took ${duration}ms (${averagePerVideo.toStringAsFixed(1)}ms per video)');
      ...
      test('concurrent requests do not interfere', () async {
        final futures = List.generate(3, (i) => 
          ThumbnailApiService.thumbnailExists('$realVideoId-concurrent-$i')
        );
    ...
    group('Size Parameter Tests', () {
      test('different thumbnail sizes work correctly', () async {
        for (final size in ThumbnailSize.values) {
          ...
          print('${size.name} thumbnail URL: $url');
        }
      });

      test('fallback respects size parameter', () async {
        final url = await ThumbnailApiService.getThumbnailWithFallback(
          realVideoId,
        ...
        if (url != null) {
          ...
          print('Large thumbnail URL: $url');
        }
      }, timeout: const Timeout(Duration(seconds: 30)));
    });

    group('Edge Case Tests', () {
      test('handles very early timestamp (0.1s)', () async {
        final url = await ThumbnailApiService.getThumbnailWithFallback(
          realVideoId,
          timeSeconds: 0.1,
        );
      ...
      test('handles large timestamp (beyond video length)', () async {
        final url = await ThumbnailApiService.getThumbnailWithFallback(
          realVideoId,
          timeSeconds: 9999.0,
        );
