...
void main() {
  group('Vine Tag Integration Tests', () {
    ...
    test('UserProfileService filters should include vine tag', () {
      ...
      final filter = Filter(
        kinds: [0],
        authors: ['fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210'],
        h: ['vine'],
        limit: 1,
      );
      ...
      expect(filter.authors, equals(['fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210']));
      
      // Test JSON serialization includes vine tag
      final json = filter.toJson();
      expect(json['#h'], equals(['vine']));
      expect(json['kinds'], equals([0]));
      expect(json['authors'], equals(['fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210']));
    });
    ...
    test('SocialService reaction filters should include vine tag', () {
      ...
      expect(filter.e, equals(['f6789012345678901234567890123456789012345678901234567890abcdef12']));
      
      // Test JSON serialization includes vine tag
      final json = filter.toJson();
      expect(json['#h'], equals(['vine']));
      expect(json['kinds'], equals([7]));
      expect(json['#e'], equals(['f6789012345678901234567890123456789012345678901234567890abcdef12']));
    });
    ...
    test('Contact list filters should validate vine tag requirement', () {
      ...
      final validContacts = Event(
        ...
        [
          ['h', 'vine'],
          ['p', '9012345678901234567890123456789012345678901234567890abcdef1234567'],
          ['p', '012345678901234567890123456789012345678901234567890abcdef12345678'],
        ],
        '',
        createdAt: DateTime.now().millisecondsSinceEpoch ~/ 1000,
      );
    ...
    test('Complex filter with multiple criteria and vine tag', () {
      final complexFilter = Filter(
        kinds: [22],
        authors: ['3333333333333333333333333333333333333333333333333333333333333333', '2345678901234567890123456789012345678901234567890abcdef1234567890'],
        h: ['vine'],
        t: ['funny', 'viral'],
        limit: 25,
        since: DateTime.now().millisecondsSinceEpoch ~/ 1000 - 86400, // Last 24 hours
      );
