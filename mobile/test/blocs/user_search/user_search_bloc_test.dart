...
void main() {
  group('UserSearchBloc', () {
    ...
    group('localContacts merge', () {
      const debounceDuration = Duration(milliseconds: 400);

      blocTest<UserSearchBloc, UserSearchState>(
        'merges local contacts with network results, network takes precedence',
        setUp: () {
          when(
            () => mockProfileRepository.searchUsers(
              query: 'alice',
        ...
        act: (bloc) => bloc.add(
          UserSearchQueryChanged(
            ...
            localContacts: [
              ...
              createTestProfile('b' * 64, 'Bob Local'),
            ],
          ),
        ),
        wait: debounceDuration,
        expect: () => [
          // Loading state includes local contacts
          isA<UserSearchState>()
              .having((s) => s.status, 'status', UserSearchStatus.loading)
              .having((s) => s.results, 'results', hasLength(2)),
          // Success state has merged: network Alice + local Bob
          isA<UserSearchState>()
              .having((s) => s.status, 'status', UserSearchStatus.success)
              .having((s) => s.results, 'results', hasLength(2))
              .having(
      ...
      blocTest<UserSearchBloc, UserSearchState>(
        'preserves local contacts on failure',
        setUp: () {
          when(
            () => mockProfileRepository.searchUsers(
              query: 'error',
        ...
        act: (bloc) => bloc.add(
          UserSearchQueryChanged(
            'error',
            localContacts: [createTestProfile('a' * 64, 'Alice Local')],
          ),
        ),
        wait: debounceDuration,
        expect: () => [
      ...
      blocTest<UserSearchBloc, UserSearchState>(
        'returns only network results when no local contacts provided',
        setUp: () {
          when(
            () => mockProfileRepository.searchUsers(
              query: 'alice',
