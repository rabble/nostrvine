...
void main() {
  ...
  // #6480: draftStorageServiceProvider bakes ownerPubkey into the instance and
  // rebuilds on every account change, but this bloc's app-shell
  // BlocProvider.create runs once. Without the swap it keeps writing drafts
  // under the account the user has already left.
  group('draft storage identity swap', () {
    late _MockDraftStorageService nextDraftStorageService;
    late StreamController<DraftStorageService> draftStorageController;

    setUp(() {
      nextDraftStorageService = _MockDraftStorageService();
      draftStorageController =
          StreamController<DraftStorageService>.broadcast();
      when(
        () => nextDraftStorageService.deleteDraft(any()),
      ).thenAnswer((_) async {});
      when(
        () => nextDraftStorageService.updatePublishStatus(
          draftId: any(named: 'draftId'),
          status: any(named: 'status'),
          publishError: any(named: 'publishError'),
        ),
    ...
    tearDown(() => draftStorageController.close());

    BackgroundPublishBloc buildSwappableBloc() => BackgroundPublishBloc(
      videoPublishServiceFactory: defaultVieoPublishServiceFactory,
      draftStorageService: mockDraftStorageService,
      draftStorageStream: draftStorageController.stream,
    );

    test('routes draft writes to the swapped-in service', () async {
      final draft = _MockVineDraft();
      when(() => draft.id).thenReturn('1');
      when(() => draft.sourceDraftId).thenReturn(null);

      final bloc = buildSwappableBloc();
      addTearDown(bloc.close);

      draftStorageController.add(nextDraftStorageService);
      await Future<void>.delayed(Duration.zero);

      bloc.add(BackgroundPublishVanished(draftId: '1'));
      await Future<void>.delayed(Duration.zero);
    ...
    test('keeps using the original service until one is emitted', () async {
      final bloc = buildSwappableBloc();
      addTearDown(bloc.close);
      await Future<void>.delayed(Duration.zero);
    ...
    test(
      'a bloc built without a stream still writes through its service',
      () async {
