...
void main() {
  group('NotificationTargetResolver', () {
    ...
    test('returns same id when target is already a video id', () async {
      when(() => videoEventService.getVideoById('video_1')).thenReturn(
        null,
      );
      when(() => nostrClient.fetchEventById('video_1')).thenAnswer(
        (_) async => Event('a' * 64, 22, const [], 'video', createdAt: 1),
      );
