...
void main() {
  ...
  group('resolveEffectiveContentLabels', () {
    ...
    group('community labels', () {
      test('adds a community label absent from other sources', () {
        expect(
          resolveEffectiveContentLabels(
            _video(),
            communityLabels: {'gambling'},
          ),
          equals(['gambling']),
        );
      });

      test('normalizes community label values', () {
        expect(
          resolveEffectiveContentLabels(
            _video(),
            communityLabels: {'NSFW'},
          ),
          equals(['nudity']),
        );
      });

      test('de-duplicates a community label against a creator self-label', () {
        expect(
          resolveEffectiveContentLabels(
            _video(contentWarningLabels: ['gambling']),
            communityLabels: {'gambling'},
          ),
          equals(['gambling']),
        );
  ...
  group('resolveEffectiveContentLabelsWithProvenance', () {
    test('tags creator self-labels with creator provenance', () {
      final result = resolveEffectiveContentLabelsWithProvenance(
        _video(contentWarningLabels: ['nudity']),
      );
      expect(
        result,
        equals([(value: 'nudity', provenance: ContentLabelProvenance.creator)]),
      );
    });

    test('tags community-only labels with community provenance', () {
      final result = resolveEffectiveContentLabelsWithProvenance(
        _video(),
        communityLabels: {'gambling'},
      );
      expect(
        result,
        equals([
          (value: 'gambling', provenance: ContentLabelProvenance.community),
        ]),
      );
    });

    test('creator provenance wins when a community label duplicates it', () {
      final result = resolveEffectiveContentLabelsWithProvenance(
        _video(contentWarningLabels: ['gambling']),
        communityLabels: {'gambling'},
      );
      expect(
        result,
        equals([
          (value: 'gambling', provenance: ContentLabelProvenance.creator),
        ]),
      );
    });

    test(
      'trusted-labeler provenance wins over a duplicate community label',
      () {
        final video = _video(id: 'evt-prov');
        final service = _FakeModerationLabelService(
          byEventId: {
            'evt-prov': [_label('violence')],
          },
        );
        final result = resolveEffectiveContentLabelsWithProvenance(
          video,
          moderationLabelService: service,
          communityLabels: {'violence'},
        );
        expect(
          result,
          equals([
            (
              value: 'violence',
              provenance: ContentLabelProvenance.trustedLabeler,
            ),
          ]),
        );
      },
