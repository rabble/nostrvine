  ...
  group('ThumbnailApiService', () {
    group('getThumbnailUrl', () {
      test('generates correct URL with default parameters', () {
        ...
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=2.5'));
      });

      test('generates correct URL with custom time', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-video-id', timeSeconds: 5.0);
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=5.0'));
      });
      ...
      test('generates correct URL with small size', () {
        ...
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=2.5&size=small'));
      });
      ...
      test('generates correct URL with large size', () {
        ...
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=2.5&size=large'));
      });
      ...
      test('handles special characters in video ID', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-video-id-with-dashes');
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id-with-dashes?t=2.5'));
      });

      test('handles complex video ID', () {
        final url = ThumbnailApiService.getThumbnailUrl('87444ba2b07f28f29a8df3e9b358712e434a9d94bc67b08db5d4de61e6205344');
        expect(url, equals('https://api.openvine.co/thumbnail/87444ba2b07f28f29a8df3e9b358712e434a9d94bc67b08db5d4de61e6205344?t=2.5'));
      });

      test('handles zero time', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-video-id', timeSeconds: 0.0);
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=0.0'));
      });

      test('handles decimal time', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-video-id', timeSeconds: 3.7);
        expect(url, equals('https://api.openvine.co/thumbnail/test-video-id?t=3.7'));
      });
      ...
      test('handles all size combinations correctly', () {
        ...
        for (final testCase in testCases) {
          ...
          expect(url, equals('https://api.openvine.co/thumbnail/test-id?t=1.0$expectedSuffix'));
        }
      });
    });
    ...
    group('ThumbnailSize enum', () {
      ...
      test('has all expected values', () {
        final values = ThumbnailSize.values;
    ...
    group('ThumbnailApiException', () {
      test('creates exception with message only', () {
        final exception = ThumbnailApiException('Test error');
        expect(exception.message, equals('Test error'));
        expect(exception.statusCode, isNull);
        expect(exception.toString(), equals('ThumbnailApiException: Test error'));
      });

      test('creates exception with message and status code', () {
        final exception = ThumbnailApiException('Test error', 404);
        expect(exception.message, equals('Test error'));
        expect(exception.statusCode, equals(404));
        expect(exception.toString(), equals('ThumbnailApiException: Test error (HTTP 404)'));
      });

      test('handles various status codes', () {
        final testCases = [
          {'code': 400, 'expected': 'ThumbnailApiException: Bad request (HTTP 400)'},
          {'code': 500, 'expected': 'ThumbnailApiException: Server error (HTTP 500)'},
          {'code': 404, 'expected': 'ThumbnailApiException: Not found (HTTP 404)'},
        ];
        ...
        for (final testCase in testCases) {
          ...
          final exception = ThumbnailApiException(
            ...
            code == 500 ? 'Server error' : 'Not found', 
            code
          );
    ...
    group('Parameter validation', () {
      ...
      test('handles negative time (should still work)', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-id', timeSeconds: -1.0);
        expect(url, equals('https://api.openvine.co/thumbnail/test-id?t=-1.0'));
      });

      test('handles very large time values', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-id', timeSeconds: 99999.99);
        expect(url, equals('https://api.openvine.co/thumbnail/test-id?t=99999.99'));
      });
    });
    ...
    group('Base URL consistency', () {
      test('all URLs use the same base URL', () {
        final urls = [
          ...
          ThumbnailApiService.getThumbnailUrl('test2', size: ThumbnailSize.large),
          ThumbnailApiService.getThumbnailUrl('test3', timeSeconds: 10.0),
        ];
      ...
      test('URLs are properly formatted', () {
        ...
        expect(url, matches(r'^https://api\.openvine\.co/thumbnail/[\w\-]+\?t=\d+(\.\d+)?(&size=\w+)?$'));
      });
    });
    ...
    group('Edge cases', () {
      ...
      test('handles extreme decimal precision', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-id', timeSeconds: 1.23456789);
        expect(url, contains('t=1.23456789'));
      });

      test('handles very small time values', () {
        final url = ThumbnailApiService.getThumbnailUrl('test-id', timeSeconds: 0.001);
