...
void main() {
  group('Profile Editing Race Condition Fix', () {
    ...
    setUp(() {
      ...
      testPubkey = '78a5c21b5166dc1474b64ddf7454bf79e6b5d6b4a77148593bf1e866b73c2738';
    ...
    test('should retry until updated profile is fetched', () async {
      ...
      final result = await AsyncUtils.retryWithBackoff(
        operation: () async {
          ...
          final profile = await mockUserProfileService.fetchProfile(testPubkey, forceRefresh: true);
          ...
          final timestampMatches = profile?.createdAt != null && 
                                 profile!.createdAt.millisecondsSinceEpoch >= (testTimestamp * 1000 - 1000);
      ...
      verify(mockUserProfileService.fetchProfile(testPubkey, forceRefresh: true)).called(3);
    });

    test('should succeed immediately if first fetch returns updated profile', () async {
      ...
      verify(mockUserProfileService.fetchProfile(testPubkey, forceRefresh: true)).called(1);
    });
    ...
    test('should validate timestamp match correctly', () async {
      ...
      expect(result?.createdAt.millisecondsSinceEpoch, greaterThan(testTimestamp * 1000 - 1000));
    });
  });
}