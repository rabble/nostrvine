...
void main() {
  group('CurationService Editor\'s Picks', () {
    ...
    test('should show videos from Classic Vines pubkey in Editor\'s Picks', () {
      ...
      final classicVineVideos = List.generate(5, (index) => VideoEvent(
        id: 'classic_$index',
        pubkey: AppConstants.classicVinesPubkey,
        createdAt: DateTime.now().subtract(Duration(days: index)).millisecondsSinceEpoch,
        content: 'Classic Vine $index',
        timestamp: DateTime.now().subtract(Duration(days: index)),
        videoUrl: 'https://example.com/classic_$index.mp4',
      ));

      final regularVideos = List.generate(3, (index) => VideoEvent(
        id: 'regular_$index',
        pubkey: 'other_pubkey_$index',
        createdAt: DateTime.now().subtract(Duration(hours: index)).millisecondsSinceEpoch,
      ...
      expect(editorsPicks.every((video) => video.pubkey == AppConstants.classicVinesPubkey), isTrue);
      expect(editorsPicks.every((video) => video.id.startsWith('classic_')), isTrue);
    ...
    test('should randomize Classic Vines order in Editor\'s Picks', () {
      ...
      final classicVineVideos = List.generate(10, (index) => VideoEvent(
        id: 'classic_$index',
        pubkey: AppConstants.classicVinesPubkey,
        createdAt: DateTime.now().subtract(Duration(days: index)).millisecondsSinceEpoch,
        content: 'Classic Vine $index',
      ...
      final hasDifferentOrder = orders.any((order) => 
        order.length == firstOrder.length && 
        !order.asMap().entries.every((entry) => entry.value == firstOrder[entry.key])
      );
      
      expect(hasDifferentOrder, isTrue, 
