...
StartupCoordinator _createStartupCoordinator(ProviderContainer container) {
  ...
  // no-op handle. In `deferred` that silently cost us every cold-start
  // measurement, including six of the eight `feed_load_*` feeds, which mount
  // while the later phases are still working through their queue (#7118).
  // Cost here is a single platform round-trip that runs in parallel with the
  // disk-backed services above, so it does not extend the phase.
  coordinator.registerService(
