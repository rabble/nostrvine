...
class LoggingConfigService {
  ...
  Future<void> initialize() async {
    try {
      ...
      if (storedLevel != null) {
        ...
        Log.info('Logging initialized with stored level: ${level.name}', name: 'LogConfig');
      } else {
        // Log initial level
        Log.info('Logging initialized with default level: ${UnifiedLogger.currentLevel.name}', name: 'LogConfig');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  void enableVerboseLogging() {
    ...
    Log.warning('Verbose logging enabled - remember to disable for production!', name: 'LogConfig');
  }
