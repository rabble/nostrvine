...
class ThumbnailApiService {
  ...
  static Future<bool> thumbnailExists(String videoId) async {
    try {
      ...
      Log.debug('Thumbnail existence check for $videoId: ${response.statusCode}', 
          name: 'ThumbnailApiService', category: LogCategory.api);
      
      return response.statusCode == 200;
    } catch (e) {
      Log.error('Error checking thumbnail existence for $videoId: $e', 
          name: 'ThumbnailApiService', category: LogCategory.api);
      return false;
    }
  }
  ...
  static Future<bool> uploadCustomThumbnail(
    ...
    try {
      ...
      Log.info('Uploading custom thumbnail for video $videoId', 
          name: 'ThumbnailApiService', category: LogCategory.api);
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        Log.info('Successfully uploaded custom thumbnail for $videoId', 
            name: 'ThumbnailApiService', category: LogCategory.api);
        return true;
      } else {
        final responseBody = await response.stream.bytesToString();
        Log.error('Failed to upload custom thumbnail for $videoId: ${response.statusCode} $responseBody', 
            name: 'ThumbnailApiService', category: LogCategory.api);
        return false;
      }
    } catch (e) {
      Log.error('Error uploading custom thumbnail for $videoId: $e', 
          name: 'ThumbnailApiService', category: LogCategory.api);
      return false;
    }
  }
  ...
  static Future<String?> getThumbnailWithFallback(
    ...
    try {
      Log.debug('🚀 getThumbnailWithFallback called for video ${videoId.substring(0, 8)} (timeSeconds: $timeSeconds, size: $size)', 
          name: 'ThumbnailApiService', category: LogCategory.video);
      
      // Backend automatically generates thumbnails on-demand via GET endpoint
      final thumbnailUrl = getThumbnailUrl(videoId, timeSeconds: timeSeconds, size: size);
      Log.info('✅ Returning thumbnail URL for ${videoId.substring(0, 8)}: $thumbnailUrl', 
          name: 'ThumbnailApiService', category: LogCategory.video);
      
      return thumbnailUrl;
    } catch (e) {
      Log.error('❌ Error in getThumbnailWithFallback for ${videoId.substring(0, 8)}: $e', 
          name: 'ThumbnailApiService', category: LogCategory.video);
      return null;
    }
  }
  ...
  static Future<Map<String, String>> batchGenerateThumbnails(
    ...
    Log.info('Batch generating thumbnails for ${videoIds.length} videos', 
        name: 'ThumbnailApiService', category: LogCategory.api);
    ...
    for (int i = 0; i < videoIds.length; i += batchSize) {
      ...
      final futures = batch.map((videoId) async {
        final url = await getThumbnailWithFallback(videoId, timeSeconds: timeSeconds);
        if (url != null) {
          results[videoId] = url;
        }
      });
    ...
    Log.info('Batch generation completed: ${results.length}/${videoIds.length} successful', 
        name: 'ThumbnailApiService', category: LogCategory.api);
...
class ThumbnailApiException implements Exception {
  ...
  @override
  String toString() => 'ThumbnailApiException: $message${statusCode != null ? ' (HTTP $statusCode)' : ''}';
}