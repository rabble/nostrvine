...
class HashtagExtractor {
  ...
  static List<String> extractHashtags(String text) {
    ...
    final RegExp hashtagRegex = RegExp(r'#(\w+)', caseSensitive: false);
  ...
  static bool isValidHashtag(String hashtag) {
    ...
    if (!RegExp(r'^[a-zA-Z]').hasMatch(hashtag)) {
      return false;
    }
  ...
  static List<String> normalizeHashtags(List<String> hashtags) {
    ...
    for (final hashtag in hashtags) {
      final cleaned = hashtag.toLowerCase().replaceAll(RegExp(r'[^a-z0-9_]'), '');
  ...
  static HashtagStats getHashtagStats(List<String> hashtags) {
    return HashtagStats(
      ...
      averageLength: hashtags.isEmpty 
        ? 0.0 
        : hashtags.map((tag) => tag.length).reduce((a, b) => a + b) / hashtags.length,
...
class HashtagStats {
  ...
  @override
  String toString() {
    return 'HashtagStats(total: $totalCount, valid: $validCount, '
