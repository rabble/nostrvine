...
| `DivineSnackbarContainer` | Action, secondary action, and dismiss button all meet the 48dp / 44pt tap-target minimum (the action's `minimumSize` is 72×48), and every tappable node carries a name — `dismissSemanticLabel` defaults to `Dismiss`. Label and action text meet 4.5:1 in both appearances, in default and `error` variants. `liveRegion` is deliberately **not** set on the container: every call site renders it as a Material `SnackBar`'s `content`, and `SnackBar` already sets one; a nested second live region would double-announce. |
