...
| `DiVineAppBarActions` / `DivineAppBarIconButton` | Actions meet the 48dp / 44pt tap-target minimum, and a `semanticLabel` on `DiVineAppBarAction` reaches the rendered button. Pass `semanticLabel` (or `tooltip`) for an accessible name; `semanticIdentifier` is the E2E anchor. |
