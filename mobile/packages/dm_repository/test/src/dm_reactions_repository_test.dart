...
void main() {
  ...
  group(DmReactionsRepository, () {
    ...
    group('expirePendingReaction (#8443)', () {
      test(
        'flips the row under the signed-in owner and reports that it changed',
        () async {
          when(
            () => mockDao.expirePending(
              id: _reactionRumorId,
              ownerPubkey: _ownerPubkey,
            ),
          ).thenAnswer((_) async => 1);

          final changed = await createRepository().expirePendingReaction(
            rumorId: _reactionRumorId,
          );
      ...
      test('reports false when the row was no longer pending', () async {
        when(
          () => mockDao.expirePending(
            id: _reactionRumorId,
            ownerPubkey: _ownerPubkey,
      ...
      test('does nothing without credentials', () async {
        final changed = await createRepository(
          initialized: false,
        ).expirePendingReaction(rumorId: _reactionRumorId);
