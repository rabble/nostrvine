...
void main() {
  group(DmSyncState, () {
    ...
    group('readRelayFingerprint / recordReadRelays (#8439)', () {
      const relayA = 'wss://relay.divine.video';
      const relayB = 'wss://relay.example.com';

      test('defaults to empty so a fresh install has nothing to compare '
          'against', () {
        expect(state.readRelayFingerprint(pkA), isEmpty);
      });

      test('records relays and returns them as the delta', () async {
        final delta = await state.recordReadRelays(pkA, [relayA, relayB]);

        expect(delta, {relayA, relayB});
        expect(state.readRelayFingerprint(pkA), containsAll([relayA, relayB]));
      });

      test('a relay already recorded is not returned again, so an exposure '
          'can only fire once for it', () async {
        await state.recordReadRelays(pkA, [relayA]);
      ...
      test('a second call naming nothing new returns an empty delta', () async {
        await state.recordReadRelays(pkA, [relayA, relayB]);

        expect(await state.recordReadRelays(pkA, [relayA, relayB]), isEmpty);
      });

      test(
        'a trailing-slash variant is the SAME relay, not a new one',
        () async {
          // #8377 is the reason this is pinned: RelayPool keys on the stripped
          // form while handleAddrList appends a slash, so a raw string compare
          // matched nothing. Re-deriving the identity here instead of reusing
          // normalizeRelayUrl would report the same relay as new forever.
          await state.recordReadRelays(pkA, [relayA]);

          expect(await state.recordReadRelays(pkA, ['$relayA/']), isEmpty);
        },
      );

      test('a bare host is upgraded and matches its wss form', () async {
        await state.recordReadRelays(pkA, [relayA]);

        expect(
          await state.recordReadRelays(pkA, ['relay.divine.video']),
          isEmpty,
        );
      });

      test('an unusable relay URL is dropped rather than stored raw', () async {
        final delta = await state.recordReadRelays(pkA, [
          'http://relay.divine.video',
          '',
          relayA,
        ]);

        expect(delta, {relayA});
        expect(state.readRelayFingerprint(pkA), [relayA]);
      });

      test('stops growing at the cap instead of evicting', () async {
        // Evicting would forget a relay and then report it as new again on the
        // next rotation, turning a one-shot signal into a recurring one.
        final many = List.generate(
          DmSyncState.maxTrackedReadRelays + 5,
          (i) => 'wss://relay$i.example.com',
        );

        final first = await state.recordReadRelays(pkA, many);

        expect(first, hasLength(DmSyncState.maxTrackedReadRelays));
        expect(
          state.readRelayFingerprint(pkA),
          hasLength(DmSyncState.maxTrackedReadRelays),
        );
        expect(await state.recordReadRelays(pkA, many), isEmpty);
      });

      test('is scoped per pubkey', () async {
        await state.recordReadRelays(pkA, [relayA]);

        expect(state.readRelayFingerprint(pkB), isEmpty);
        expect(await state.recordReadRelays(pkB, [relayA]), {relayA});
      });

      test('clear removes the fingerprint for that pubkey only', () async {
        await state.recordReadRelays(pkA, [relayA]);
        await state.recordReadRelays(pkB, [relayB]);

        await state.clear(pkA);

        expect(state.readRelayFingerprint(pkA), isEmpty);
        expect(state.readRelayFingerprint(pkB), [relayB]);
      });

      test('clearAll removes every fingerprint', () async {
        await state.recordReadRelays(pkA, [relayA]);
        await state.recordReadRelays(pkB, [relayB]);
