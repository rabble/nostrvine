...
void main() {
  group('Apple native player threading contract', () {
    test('sends Flutter event updates on the main thread', () {
    ...
    test('uses native queue-based looping', () {
      final source = _appleSourceFile().readAsStringSync();
    ...
    test('reports requested paused position during texture refresh', () {
      final source = _appleSourceFile().readAsStringSync();
    ...
    test('emits freeze diagnostics for stalled native playback', () {
      final source = _appleSourceFile().readAsStringSync();
...
/// (`darwin/divine_video_player/Sources/`), so the threading contract
/// is asserted once.
