...
void main() {
  ...
  group('DmReactionsDao', () {
    ...
    test(
      'expirePending flips a live pending row to failed and keeps its rumor',
      () async {
        await insertPending();

        final changed = await dao.expirePending(
          id: _pendingId,
          ownerPubkey: _ownerA,
        );
    ...
    test(
      'expirePending leaves a row that has since landed untouched',
      () async {
        await insertPending();
        await dao.swapPlaceholderId(
          placeholderId: _pendingId,
        ...
        // A manual retry can land the row between the sweep's read and this
        // write; overwriting `sent` here would leave a red chip whose Retry
        // has no stored rumor to replay.
    ...
    test(
      'expirePending leaves a soft-deleted deletion_pending row untouched',
      () async {
        await insertPending();
        ...
        expect(
          ...
          equals('deletion_pending'),
        );
      },
    );

    test('expirePending is owner scoped', () async {
      await insertPending();
