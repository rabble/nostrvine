...
on a shared baseline grid in the top third, and places the raw device capture
below inside a rounded iPhone body. `09_endcard` is a typographic brand card
(no capture, carries the only wordmark in the set).
...
def draw_caption(canvas, draw, cw, ch, headline, subhead):
    """Caption on a shared baseline grid: headline top edge and subhead top
    edge sit at the same y on every slide (headlines are <=2 lines, so a fixed
    subhead y never collides)."""
    if not headline:
        return
    head_font = ImageFont.truetype(FONT_PATH, int(cw * 0.068))
    head_lines = wrap(draw, headline, head_font, cw - 150)
    head_top = int(ch * 0.052)
    draw_centered(draw, head_lines, head_font, head_top, GREEN, cw, 6)
    if subhead:
        sub_font = ImageFont.truetype(FONT_PATH, int(cw * 0.033))
        sub_lines = wrap(draw, subhead, sub_font, cw - 180)
        sub_top = int(ch * 0.140)
...
def build_captured(shot_path, out_path, device, headline, subhead):
    ...
    draw_caption(canvas, draw, cw, ch, headline, subhead)
...
def build_endcard(out_path, device, headline, subhead):
    ...
    head_lines = sentence_lines(headline)
    sub_font = ImageFont.truetype(FONT_PATH, int(cw * 0.040))
    sub_lines = wrap(draw, subhead, sub_font, cw - 200) if subhead else []

    hlh = line_height(head_font)
    slh = line_height(sub_font)
    gap = int(ch * 0.02)
    block = len(head_lines) * (hlh + 8) + (
        gap + len(sub_lines) * (slh + 6) if sub_lines else 0
    )
        ...
        for device in sorted(devices):
            for name, src in SLIDES:
                ...
                if not os.path.exists(shot):
                    print(f"SKIP {locale}/{device}-{name}: missing {src}.png")
                    continue
