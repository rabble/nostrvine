#!/usr/bin/env bash
# Nostr-ID log-truncation ratchet (#3372): a Nostr identifier that is SHORTENED
# on its way into a log/debug sink is frozen at its current count per file in
...
#   • SECRETS are not covered by "log the full value", and nothing here asks for
#     that. An nsec is not a Nostr ID to be preserved; it is a credential to be
#     omitted. Truncation is not a security control either way — redaction is.
...
#   dart run scripts/lib/nostr_id_log_truncation_detector.dart lib packages --detail
...
NEW_HINT="Log the identifier whole. The full value is in scope at every site this has ever fired on — drop the .substring()/preview local, or the mask helper, and interpolate the value itself. If the value is a SECRET (nsec, ncryptsec, a signing key), do not shorten it either: omit it, or redact it whole via BugReportConfig.sensitivePatterns / sanitizeForCrashReport. For UI, shorten at the widget with maxLines + TextOverflow.ellipsis so the copied value stays exact. AGENTS.md → Nostr And Async Rules; #3372."
STALE_HINT="A file stopped shortening a Nostr identifier on its way into a log."
FOOTER="Shortened Nostr identifiers reaching a log/debug sink are frozen at zero.
...
# mobile/integration_test that shorten a Nostr identifier on its way into a
# log/debug sink, each with its count as a CEILING (format: relpath<TAB>count).
...
# than regenerating this file: the full value is in scope at every site.
# UI-side shortening is NOT counted (display sink, not a log sink), and secrets
# are redacted whole rather than shortened — see the script header.
# Issue: #3372.
EOF
}
