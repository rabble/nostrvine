...
# that calls a raw `showDialog(` or `showModalBottomSheet(` is frozen at its
...
#   \b(showDialog|showModalBottomSheet)\b
# Matches the bare identifier (NOT requiring a `(`) on purpose: these are generic
# functions almost always called with an explicit type argument —
# showDialog<bool>(, showModalBottomSheet<void>(, nested/multi-line generics — that
# a `...\(` regex would miss (and a dev could add a type arg to bypass). Word
# boundaries exclude wrapper NAMES (showForgotPasswordDialog, _showDialog,
# showVideoPausingVineBottomSheet); a wrapper's body still calls the raw token once,
# counted once (correct). Dartdoc refs like `/// [showModalBottomSheet]` are dropped
# by the full-line-comment filter. VineBottomSheet (the sanctioned sheet) lives in
...
DETECT_RE='\b(showDialog|showModalBottomSheet)\b'

NEW_HINT="Don't add raw showDialog/showModalBottomSheet — prefer a full-screen flow or the VineBottomSheet family from divine_ui. ui_theming.md / AGENTS.md; #6145."
STALE_HINT="A file dropped its raw dialog/sheet call (migrated to a full-screen flow / VineBottomSheet or removed)."
FOOTER="Raw showDialog/showModalBottomSheet counts under mobile/lib are frozen and
may only decrease. Prefer full-screen flows or VineBottomSheet rather than adding
more. See ui_theming.md / AGENTS.md and epics #6145 / #4339."

# Print "relpath<TAB>count" for every non-generated Dart file under SCAN_DIR that
# references a raw dialog/sheet function (outside full-line comments), by path.
emit_current() {
  find "$SCAN_DIR" \
    -type f -name '*.dart' \
...
print_baseline_header() {
  cat <<EOF
# Frozen baseline: Dart files under mobile/lib calling a raw showDialog( or
# showModalBottomSheet(, each with its current count as a CEILING (format:
...
# ${BASE_REF}. Prefer full-screen flows or VineBottomSheet. Full-line comments are
# not counted. Issue: #6145. Regenerate after migrating:
