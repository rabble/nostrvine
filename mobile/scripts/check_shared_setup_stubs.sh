...
# groups that declare tests. A leaf group's own setUp is NOT counted — the stub
# and the tests it governs are adjacent, which is the shape this pushes work
# toward. See scripts/lib/shared_setup_stub_detector.dart for the full rule.
#
...
NEW_HINT="A shared setUp must not gain stubs, and a file must not cross ${MIN_COUNT} inherited stubs. Declare the stub in the groups that depend on it — a named helper called as setUp(stubFooAllows); keeps it to one line per group. See packages/dm_repository/test/src/dm_repository_test.dart and #8399."
...
SCAN_DIRS_DEFAULT="$MOBILE_DIR/test $MOBILE_DIR/integration_test $MOBILE_DIR/packages"
...
# registers ${MIN_COUNT} or more mocktail stubs inherited by descendant groups,
