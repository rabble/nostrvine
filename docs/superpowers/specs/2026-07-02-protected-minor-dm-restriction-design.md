...
4. **Affordance guards (UX / defense-in-depth):** hide the Message button in `other_profile_screen.dart:160` for non-approved profiles when protected; guard the `ConversationPage` deep-link/route (redirect to inbox with a notice); guard the **message-request preview route** (`/inbox/message-requests/:id`) inside `RequestPreviewCubit.load()` — counterparties resolve first and the same non-empty ∩ all-approved predicate (`allParticipantsApprovedForMinor`, shared with the route guard) must pass before any request metadata (count, messages) is read, else the view bounces to the inbox. Surfaced in dcadenas's review: a stale or direct URL reached the preview without the list's filter.
