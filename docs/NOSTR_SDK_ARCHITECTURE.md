# Nostr SDK Architecture

Status: Current
Validated against: current Nostr SDK implementation on 2026-07-20.

> **Looking for the app's layer architecture** (UI → BLoC/Cubit → Repository → Client)? That lives in [docs/ARCHITECTURE.md](ARCHITECTURE.md). This document covers the **Nostr SDK internals** only — the innards of the `Client` layer, not the app's layering contract.
