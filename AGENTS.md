...
- Every key you add to `mobile/lib/l10n/app_en.arb` must be **read through `context.l10n` in the same change**. The set of keys that no non-generated Dart file references is frozen by a shrink-only ratchet (#3630): `check_orphaned_arb_key_floor.sh`, baseline `mobile/scripts/baseline/orphaned_arb_keys.txt`. An orphaned key is not free — it gets translated into 21 locales and compiled into `AppLocalizations`, while the string it was written for usually stays hardcoded in a widget and stays broken for non-English users; the set grew 94 → 276 in the four months #3630 sat open with nothing guarding it. Nothing else catches this: `arb_consistency_test` proves a key exists in every locale (an orphan passes trivially, in all 22), and `flutter analyze` cannot report an unused getter on a generated class. The detector is a real analyzer AST (`mobile/scripts/lib/orphaned_arb_key_detector.dart`, pinned by `mobile/test/tools/orphaned_arb_key_detector_test.dart`) rather than a grep, because a `// TODO wire up feedSkip`, a `Log.info('feedSkip')`, a dartdoc `[feedSkip]`, and an unrelated enum constant spelled the same way all look like references to `grep` and none of them render anything. **Baseline entries are a worklist, not exemptions**: each carries `# wire: <path>:<line>` (a live widget renders the string hardcoded — wiring it up is a real l10n fix for 21 locales) or `# staged: <why>`. A key with no live counterpart belongs in neither category — delete it from `app_en.arb` and every `app_*.arb` locale. List them with `cd mobile && dart run scripts/lib/orphaned_arb_key_detector.dart --detail`; after wiring or deleting, lock the win with `UPDATE_BASELINE=1 bash mobile/scripts/check_orphaned_arb_key_floor.sh`. Runs in CI only, in the `generated-files` job.
