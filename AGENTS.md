...
- For visual changes, run `mobile/scripts/golden.sh verify`. CI runs the same command in the `Goldens` job, over `mobile/test/goldens/` only — `scripts/ci/select_test_shard.sh` keeps that directory out of every shard of the `Tests` job. Do **not** commit references produced by a local `golden.sh update`: Skia renders differently per OS, so they must be regenerated on the runner (`gh workflow run "Mobile CI" --ref <branch> -f update_goldens=true`, then download the `goldens` artifact). Every image golden must drain google_fonts (`await tester.runAsync(GoogleFonts.pendingFonts)`) or it will silently capture Ahem blocks. See `mobile/docs/GOLDEN_TESTING_GUIDE.md`.
