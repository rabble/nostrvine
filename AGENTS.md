...
- Either Android emulator (`10.0.2.2` → host) or iOS Simulator / macOS / web (`localhost` → host) works against the local stack out of the box. `localHost` in `mobile/lib/models/environment_config.dart` resolves the right one per platform — do not hardcode either alias in app code.
- One exception: `BLOSSOM_PUBLIC_URL` is baked into the media URLs the seeder mints, so it cannot be resolved per client. It defaults to the Android emulator alias; export `BLOSSOM_PUBLIC_URL=http://localhost:43003` before `local_stack/up.sh` when running against the iOS Simulator, macOS, or web.
