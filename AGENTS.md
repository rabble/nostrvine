...
- Oversized service-layer god-files under `mobile/lib/services/` are frozen by a hard per-file line-count ceiling ratchet (#4338): `check_service_god_file_ceiling.sh`, baseline `mobile/scripts/baseline/service_god_file_sizes.txt`. The guard tracks service files over 1500 lines; a baselined file growing, a new oversized service file, a raised ceiling, or a baselined file dropping below the threshold/being renamed/being deleted fails CI, and there is no sanctioned raise path. Keep new work out of the frozen god-files by extracting responsibilities behind the existing `UI -> BLoC/Cubit -> Repository -> Client` boundaries, or shrink the touched service in the same PR. After intentionally shrinking/removing one of these files, lock the win with `UPDATE_BASELINE=1 bash mobile/scripts/check_service_god_file_ceiling.sh`, then commit the updated baseline.
