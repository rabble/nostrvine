...
- Full LOCAL app runs also need the invite server define. `local_stack/run_android_local.sh` passes `INVITE_SERVER_URL=http://10.0.2.2:43004` for the Android emulator; iOS Simulator and macOS runs should pass `--dart-define=INVITE_SERVER_URL=http://localhost:43004`.
- Web resolves local endpoints to `localhost`, but Flutter web runs are not out of the box because the browser enforces CORS and the local stack does not allow arbitrary `localhost:<dev-port>` origins.
