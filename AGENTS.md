...
- Oversized service-layer god-files under `mobile/lib/services/` are frozen by a **hard per-file line-count ceiling ratchet** (#4338): `check_service_god_file_ceiling.sh`, baseline `mobile/scripts/baseline/service_god_file_sizes.txt`. The guard tracks service files over 1500 lines; a baselined file growing, a new oversized service file, or a raised ceiling **fails CI unconditionally, and there is no sanctioned raise path**; a baselined file dropping below the threshold/being renamed/being deleted fails until the baseline is regenerated to lock the win. Keep new work out of the frozen god-files by extracting responsibilities behind the existing `UI -> BLoC/Cubit -> Repository -> Client` boundaries, or shrink the touched service in the same PR. After intentionally shrinking/removing one of these files, lock the win with `UPDATE_BASELINE=1 bash mobile/scripts/check_service_god_file_ceiling.sh`, then commit the updated baseline. Runs in CI only, in the `generated-files` job; the pre-push hook does not cover it, so do not rely on the hook to catch god-file growth. Six service files are currently baselined — `auth_service`, `curated_list_service`, `upload_manager`, `video_editor/video_editor_render_service`, `video_event_publisher`, and `video_event_service` — with `mobile/scripts/baseline/service_god_file_sizes.txt` as the authoritative list.
