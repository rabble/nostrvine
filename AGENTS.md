...
- For visual changes, run `mobile/scripts/golden.sh verify`. CI runs the same command in the `Goldens` job, over `mobile/test/goldens/` only — `scripts/ci/select_test_shard.sh` keeps that directory out of every shard of the `Tests` job. Three things differ from a normal test suite: (1) references are committed as the **Ubuntu runner** generated them, so on macOS expect a ~3% pixel diff from font antialiasing — that run catches structural breakage (size mismatches, exceptions), and CI is authoritative for pixels; (2) never commit references from a local `golden.sh update` — regenerate on the runner with `gh workflow run "Mobile CI" --ref <branch> -f update_goldens=true` and download the `goldens` artifact; (3) every image golden must drain google_fonts (`await tester.runAsync(GoogleFonts.pendingFonts)`) or it silently captures Ahem blocks instead of glyphs. See `mobile/docs/GOLDEN_TESTING_GUIDE.md`.
