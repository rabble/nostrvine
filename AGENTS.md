...
- Never truncate Nostr IDs in code, logs, tests, analytics, or debug output. Use full values and let UI layout handle overflow visually. A truncated id *looks* correlatable and is not: `npub1abc...wxyz` cannot be grepped against relay logs, matched to a funnelcake row, or pasted into a support ticket, and 8 hex characters of an event id is 32 bits. Frozen at zero by `check_nostr_id_log_truncation.sh` (#3372) over `mobile/lib`, `mobile/packages`, `mobile/test` and `mobile/integration_test`. The detector (`scripts/lib/nostr_id_log_truncation_detector.dart`) counts only a shortening that reaches a **log/debug sink**, so display shortening in the UI stays allowed — but prefer `maxLines` + `TextOverflow.ellipsis` over cutting the string, so the value a user copies is still exact.
