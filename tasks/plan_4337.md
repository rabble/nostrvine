...
The test and production baselines store `relpath<TAB>count`. A count may
decrease without immediately changing the baseline; regenerate after a paydown
to lock in the lower ceiling so the removed call cannot return. The detector
strips comments, dartdoc, and string literals before counting occurrences.
